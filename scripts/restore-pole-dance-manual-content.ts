/**
 * Przywraca moją oryginalną treść instrukcji rury pole dance do manuala
 * „Rury pole dance" (id tracked w bazie). Zachowuje branding (A5, Plus Jakarta
 * Sans, logo ACRO4F, footer) — nadpisuje tylko manualJson + headerRanges.
 *
 * Przed nadpisaniem zapisuje backup obecnej treści do scripts/.backup-*.json.
 */
...
const PAGE_MOZLIWOSCI = {
  ...
  content: [
    ...
    paragraph(
      text("Zalecamy "),
      bold("zastosowanie łożyska"),
      text(" z uwagi na większą stabilność."),
    ),
    h3("Cechy rury:"),
    bulletList(
      ...
      [
        text(
          "Można go stosować na praktycznie każdej podłodze, takiej jak płytki, drewno, dywan itp.",
        ),
      ],
...
const PAGE_KROK1 = {
  ...
  content: [
    ...
    paragraph(text("Sufit posłuży jako podpora do montażu rury.")),
    paragraph(
      bold("OPCJA 1:"),
      text(
    ...
    paragraph(
      text(
        "Zmierz odległość między podłogą, a sufitem i wybierz minimalną wysokość najbardziej zbliżoną pomiarowi.",
      ),
    ),
    paragraph(
      text(
        "Następnie wystarczy wybrać przedłużkę pokazaną w poniższej tabeli i można rozpoczynać montaż.",
      ),
    ),
    table([
      row(
      ...
      row(td("215 – 230 cm"), td("220 – 232 cm"), td("100 cm, 100 cm")),
      row(td("228 – 243 cm"), td("233 – 245 cm"), td("100 cm, 100 cm, 13,5 cm")),
      row(td("247 – 256 cm"), td("246 – 259 cm"), td("100 cm, 100 cm, 26,5 cm")),
      row(
        td("255 – 269 cm"),
        td("260 – 271 cm"),
        td("100 cm, 100 cm, 13,5 cm, 26,5 cm"),
      ),
      row(td("Powyżej 269 cm"), td("Powyżej 271 cm"), td("Dodatkowe przedłużki")),
    ]),
  ],
};
...
const PAGE_KROK4 = {
  ...
  content: [
    ...
    paragraph(text("Tryb obrotowy 360°")),
  ],
};
...
const PAGE_KONSERWACJA = {
  ...
  content: [
    ...
    bulletList(
      ...
      [
        text(
          "Czyścić ściereczką z delikatnym detergentem, następnie wytrzeć do sucha.",
        ),
      ],
    ),
    h3("OSTRZEŻENIA:"),
    ...
    paragraph(
      text("Odkręć dwie ukryte śruby tuż nad podstawą za pomocą klucza."),
    ),
...
const BODY_PAGES = [
  { id: "page-zawartosc", lang: "PL", content: PAGE_ZAWARTOSC, sourceUpdatedAt: NOW },
  { id: "page-mozliwosci", lang: "PL", content: PAGE_MOZLIWOSCI, sourceUpdatedAt: NOW },
  { id: "page-krok1", lang: "PL", content: PAGE_KROK1, sourceUpdatedAt: NOW },
  { id: "page-krok23", lang: "PL", content: PAGE_KROK23, sourceUpdatedAt: NOW },
  { id: "page-krok4", lang: "PL", content: PAGE_KROK4, sourceUpdatedAt: NOW },
  { id: "page-konserwacja", lang: "PL", content: PAGE_KONSERWACJA, sourceUpdatedAt: NOW },
];

const MANUAL_JSON = { pages: BODY_PAGES, activeLanguages: ["PL"] };

const HEADER_RANGES = [
  { id: "h-zawartosc", fromPage: 3, toPage: 3, lang: "PL", title: "Zawartość zestawu", rightText: null, rightImageUrl: null },
  { id: "h-mozliwosci", fromPage: 4, toPage: 4, lang: "PL", title: "Możliwości montażu", rightText: null, rightImageUrl: null },
  { id: "h-krok1", fromPage: 5, toPage: 5, lang: "PL", title: "Montaż krok 1", rightText: null, rightImageUrl: null },
  { id: "h-krok23", fromPage: 6, toPage: 6, lang: "PL", title: "Montaż krok 2, 3", rightText: null, rightImageUrl: null },
  { id: "h-krok4", fromPage: 7, toPage: 7, lang: "PL", title: "Montaż krok 4", rightText: null, rightImageUrl: null },
  { id: "h-konserwacja", fromPage: 8, toPage: 8, lang: "PL", title: "Konserwacja i użytkowanie", rightText: null, rightImageUrl: null },
];
...
async function main() {
  ...
  if (!m) throw new Error('Brak manuala "Rury pole dance"');
  console.log(`Manual: ${m.name}  id=${m.id}`);

  // Backup obecnej treści (na wszelki wypadek)
  const backup = { id: m.id, name: m.name, manualJson: m.manualJson, headerRanges: m.headerRanges };
  const stamp = Math.floor(Date.now() / 1000);
  const backupPath = join(__dirname, `.backup-rury-${stamp}.json`);
  ...
  await db.productManual.update({
    where: { id: m.id },
    data: {
      manualJson: MANUAL_JSON as Prisma.InputJsonValue,
      headerRanges: HEADER_RANGES as Prisma.InputJsonValue,
    },
  });
  console.log(`\n✓ Przywrócono 6 stron + 6 headerRanges`);
  await db.$disconnect();
}
